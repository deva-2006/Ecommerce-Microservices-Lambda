...
class XRayConfigTest {
    ...
    void doFilter_httpRequest_tracesRequestAndResponse() throws Exception {
        ...
        when(request.getRequestURI()).thenReturn("/inventory/prod-1");
        when(request.getMethod()).thenReturn("GET");
        when(response.getStatus()).thenReturn(200);

        try (MockedStatic<AWSXRay> xray = mockStatic(AWSXRay.class)) {
            xray.when(() -> AWSXRay.beginSubsegment("inventory-service")).thenReturn(subsegment);
    ...
    void doFilter_httpRequest_noSubsegment_skipsTracing() throws Exception {
        ...
        try (MockedStatic<AWSXRay> xray = mockStatic(AWSXRay.class)) {
            xray.when(() -> AWSXRay.beginSubsegment("inventory-service")).thenReturn(null);
    ...
    @Test
    void doFilter_chainThrows_noSubsegment_rethrows() throws Exception {
        HttpServletRequest request = mock(HttpServletRequest.class);
        HttpServletResponse response = mock(HttpServletResponse.class);
        FilterChain chain = mock(FilterChain.class);
        RuntimeException boom = new RuntimeException("boom");
